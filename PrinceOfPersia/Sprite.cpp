...
Sprite::Sprite(const glm::vec2 &quadSize, const glm::vec2 &sizeInSpritesheet, Texture *spritesheet, ShaderProgram *program)
{
	...
	spritesheetQuad = sizeInSpritesheet;
	currentAnimation = -1;
...
void Sprite::render(glm::vec2 cuadranteTranslate) const
{
	glm::vec2 posScreen = position - cuadranteTranslate;
	glm::mat4 modelview = glm::translate(glm::mat4(1.0f), glm::vec3(posScreen.x, posScreen.y, 0.f));
...
void Sprite::addKeyframe(int animId, const glm::vec2 &coordSpritesheet)
{
	if(animId < int(animations.size()))
		animations[animId].keyframeDispl.push_back(coordSpritesheet*spritesheetQuad);
}
...
bool Sprite::isLastKeyframe() const
{
